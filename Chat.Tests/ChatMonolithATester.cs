...
namespace Chat.Tests {
    public class ChatMonolithATester : BenchmarkTesterBase {
        ...
        protected override void ExecuteBenchmarkThread(IBenchmarkDatabase benchmarkDataCollection) {
            ...
            // Get chat history
            GetChatHistory(benchmarkDataCollection, "monolith", "/history", client, roomId, sender, receiver, out DateTime getHistoryStart, out float historyDuration, out BenchmarkTag historyTags);


            // Send messages
            for (int msgIdx = 0; msgIdx < msgCount; msgIdx++) {
                SendMessage(benchmarkDataCollection, "monolith", "/send", client, sender, roomId, msgIdx, receiver, out DateTime msgStart, out Task<MessageSendResponseContract> sendTask, out BenchmarkTag sendTags);
            }
        }
    }
}