...
var db = new LiteDB.LiteDatabase("../../../../Chat.Tests/bin/Debug/net9.0/benchmark.db");
var dataCollection = db.GetCollection<Data>();
...
Console.WriteLine("endpoint;duration;count;avgDuration;minDuration;maxDuration");
foreach (var idx in unIndexes) {
    ...
    foreach (var endpointData in groupedData) {
        ...
        //Console.WriteLine();
        Console.WriteLine($"'{endpointData.Endpoint}';{endpointData.AvgDuration:F2};{endpointData.Count};{endpointData.AvgDuration:F2};{endpointData.MinDuration:F2};{endpointData.MaxDuration:F2}");
    }
}

record PackedData(string endpoint, float durationMs, string sender, string receiver);